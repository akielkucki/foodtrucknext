...
export default function LoadingScreen() {
    const phrases = ["Innovative", "Reliable", "Professional", "Durable", ""];
    ...
    if (!hasVisited) {
        return (
            <AnimatePresence mode="popLayout">
                {visible && (
                    <motion.div
                        className="min-h-screen w-full fixed z-50 bg-slate-950 flex items-center justify-center flex-col text-center"
                        exit={{ opacity: 0 }}
                    >
                        {current !== phrases.length - 1 ? (
                            <motion.h1
                                ...
                                className="text-white absolute w-full z-10 text-5xl font-bold"
                            >
                                {phrases[current]}
                            </motion.h1>
                        ) : (
